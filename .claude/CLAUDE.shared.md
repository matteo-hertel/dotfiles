...
This repo keeps Claude guidance in `.claude/` and Codex guidance in `.codex/`. Change both sides in the same commit so both agents behave the same. Tool differences are listed at the end of `.codex/AGENTS.md`; add any new one there.

`CLAUDE.work.md` holds the Let's Do This work rules. Only a work machine imports it; my personal Mac doesn't.
...
Use the `AskUserQuestion` tool so I can answer in one click. If you need room to explain, write the options out first, then ask. When the tool isn't available (a subagent or a headless run), ask one short plain-text question instead.
...
- **HTML files:** put them under `tmp/` in the current folder unless I say otherwise, so the working tree stays clean. Serve the file with a small local server and open it for me.
